    ...
    async rebuildIndex() {
        ...
        try
        {
            ...
            if (item.documentId)
            {
                ...
                await scheduleIndexingService.markIndexingTaskComplete(item.documentId);    
    ...
    async indexCollection(collectionName, indexName = null) {
        ...
        if (entries)
        {
            for (let s=0; s<entries.length; s++)
            {
                ...
                const indexItemId = helper.getIndexItemId({collectionName: collectionName, itemDocumentId: item.documentId});    
                    ...
                    if (configureIndexingService.isCollectionConfiguredToBeIndexed(col))
                    {
                        ...
                        if (recs[r].item_document_id)
                        {
                            if (recs[r].indexing_type !== 'remove-from-index')
                            {
                                ...
                                const indexItemId = helper.getIndexItemId({collectionName: col, itemDocumentId: item.documentId});    
